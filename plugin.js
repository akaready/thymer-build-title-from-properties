...
`;
  ...
  var Plugin = class extends AppPlugin {
    ...
    _rootFromEvent(ev) {
      const el2 = ev && (ev.currentTarget || ev.target);
      return el2 && el2.closest ? el2.closest(`.${ROOT_CLASS}-panel`) : null;
    }
    ...
    _renderInto(root) {
      ...
      const instructions = Array.isArray(conf.instructions) ? conf.instructions : conf.instructions ? [conf.instructions] : [];
      root.replaceChildren(panel({ pluginClass: `${ROOT_CLASS}-panel` }, [
        pluginHeaderFromConfig(conf, {
          version: PLUGIN_VERSION,
          helper: [
            ...instructions,
            "Kill-switch note: title building runs through managed hooks installed into each collection plugin \u2014 the header switch pauses marker styling and hook management, but already-installed hooks keep rebuilding titles until you remove their rules."
          ],
          killSwitch: {
            on: !this._disabled,
